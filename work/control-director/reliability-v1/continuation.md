...
`roadmap.json` is authoritative. Implementation and exact-SHA certification are separate. Official total completion is certification coverage. A milestone is complete only when `implementationStatus` resolves to `implemented`, `certificationStatus` and its compatibility `status` are both `passed`, and evidence identifies current source plus every applicable Mac Studio-local test, build, Dashboard, managed-runtime, persistence, restart, soak, and rollback proof surface.

The managed Mac Studio is the sole Control Director execution and certification platform. Exact-source implementation, full local validation, Mac Studio Dashboard, managed runtime, model, restart, rollback, soak, landing, and final-ledger proof are separate gates. Blacksmith, Testbox, Crabbox, full non-Android CI, and equivalent remote execution are not Control Director certification surfaces. GitHub remains source-hosting and landing infrastructure; unavoidable repository checks are external landing constraints, not certification evidence. Do not infer completion from a plan, source-string readiness check, mock-only test, screenshot, stale receipt, prior runtime, or remote runner.
...
- M69 adds dual progress and an M01-M86 dependency graph. M70-M73 add exact lineage, accepted-head, semantic inventory, and all-blocker preflight contracts. M74 adds automatic Mac Studio-local exact-source proof. M75 emits one batched Mac Studio remediation manifest. M76-M81 harden certification leases and managed lifecycle arbitration. M82-M86 remain gated on audit, candidate freeze, exact local source/landing, Mac Studio managed-runtime proof, and final SIG closure.
...
- Current active-worktree proof after the M61/M68 hardening includes passing grouped runtime/final-ledger, custom-runtime lifecycle, preservation-digest, PCC update-safety, protocol, Chat-layout, and UI tests; passing changed core/UI/script lint; passing core, core-test, package-test, and UI-test typechecks; passing capability registries; and historical responsive browser smoke. These are pre-commit local results, not clean exact-SHA Mac Studio Dashboard or managed-runtime proof.
...
- Remaining proof: exact Mac Studio runtime and host lineage; Mac Studio Dashboard; local-model routing and latency; memory; delegation; Judge; queue/steer; Pursue Goal; restart recovery; rollback/restore; five-minute soak; landing; reactivation; final ledger.
...
- Historical full non-Android CI exposed a singular `*.test-fixture.ts` file that Knip did not recognize, two PCC lint violations, and an outdated browser assertion that treated the intentional collapsed blocked-claim diagnostic as an obstruction. Those findings remain useful diagnostic history but their remote receipts are not current certification evidence.
- The approved six-advisory production-dependency remediation is committed as exact source `ef4ee5f5f740e7c3e742241004a7001acd1477dc`. Historical Workflow Sanity run `30009893756` and full non-Android CI run `30009893801` remain preserved but do not satisfy the revised Mac Studio-local certification policy.
- The first restored final-ledger draft could accept abbreviated source, update-survival, and readiness ledgers whose only entries claimed a generic pass. The active worktree now requires the complete canonical source command set, the named M61 preservation facts, the named production-readiness facts, and explicit M61 bindings to source, update-survival, Mac Studio managed runtime, local validation, and readiness. Landing and live-diagnostic truth surfaces remain mandatory.
...
- The requested Chat profile image remains unconfigured. Preserve the supplied image outside source control, implement or use the supported per-user avatar setting only after the reliability candidate is stable, verify Mac Studio Dashboard rendering, and register any new source customization under M61 before calling it complete.

## Next dependency-ready work

1. Complete targeted M69-M81 tests, local workflow validation, formatting, lint, type, and build checks on the Mac Studio; commit and push only intended source files to the writable `SnowBelt` remote.
...
4. Freeze M83 and run exact-SHA Control Director verification, local workflow sanity, full local source/test/build validation, Mac Studio Dashboard proof, and bounded local independent review without a remote-execution dependency.
...
- Do not mark a customization update-safe unless its capability/path inventory, exact-parent candidate proof, loaded prepare-only update broker and runtime recovery guard, Mac Studio managed activation, Dashboard proof, rollback/restore, and soak all pass.
