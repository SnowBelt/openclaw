...
async function main() {
  const artifactDir = join(
    ".artifacts",
    "control-ui-pcc-final-functionality-closure-smoke",
    timestampSlug(),
  );
  ...
  try {
          ...
          receipts: [
            {
              ...
              proofLevel: "production" as const,
              proofEvidenceIds: ["proof", "missing-old-tmp-proof"],
              artifactRefs: [],
              doNotRedo: [],
              followUpGaps: [],
            },
    ...
    const summary = {
      ...
      checks,
      calls,
      html: join(artifactDir, "pcc-final-functionality-closure.html"),
    };
