...
function requireText(text: string, label: string): void {
  if (!text.includes(label)) {
    throw new Error(`PCC skimmable journey smoke missing text: ${label}`);
  }
}

function requireSelector(root: ParentNode, selector: string): void {
  if (!root.querySelector(selector)) {
    throw new Error(`PCC skimmable journey smoke missing selector: ${selector}`);
  }
}

async function main(): Promise<void> {
  const artifactDir = join(".artifacts", "control-ui-pcc-skimmable-journey-smoke", stamp());
  ...
  try {
    ...
    requireSelector(root, "[data-pcc-journey-phase]");
    ...
    if (!cancelButton) {
      throw new Error("PCC skimmable journey smoke missing Create Project cancel button");
    }
