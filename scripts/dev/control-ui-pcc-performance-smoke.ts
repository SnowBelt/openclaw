...
const CAPABILITY_REQUIREMENT_COUNT = 1_000;
const CAPABILITY_NOISE_COUNT = 5_000;
const PERFORMANCE_BUDGETS = {
  capabilityResolutionP95Ms: 25,
  gatewaySummaryP95Ms: 80,
  templateP95Ms: 40,
  initialDomMs: 900,
  rerenderP95Ms: 75,
  searchDomMs: 200,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const { buildPccCapabilityContract, resolvePccCapabilityContract } =
      await import("../../src/pcc/capability-contract.ts");
    ...
    const requiredSkills = Array.from(
      { length: CAPABILITY_REQUIREMENT_COUNT },
      (_, index) => `required-skill-${index}`,
    );
    const capabilityContract = buildPccCapabilityContract("software-product", {
      pccRequiredSkills: requiredSkills,
    });
    const capabilityInventory = [
      ...Array.from({ length: CAPABILITY_NOISE_COUNT }, (_, index) => ({
        id: `noise-skill-${index}`,
        kind: "skill" as const,
    ...
    const capabilityResolutionTimes = Array.from(
      { length: 12 },
      () =>
        timed(() =>
    ...
    const metrics = {
      ...
      capabilityRequirementCount: CAPABILITY_REQUIREMENT_COUNT,
      capabilityInventoryCount: capabilityInventory.length,
      capabilityResolutionP95Ms: Number(percentile(capabilityResolutionTimes, 0.95).toFixed(2)),
