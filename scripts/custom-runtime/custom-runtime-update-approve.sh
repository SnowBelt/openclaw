...
[ -f "$active_pointer" ] || { printf '%s\n' 'active runtime pointer is missing' >&2; exit 64; }
active_pointer_sha256=$(shasum -a 256 "$active_pointer" | awk '{print $1}') || {
  printf '%s\n' 'active runtime pointer digest is unavailable' >&2
  exit 64
}

fields=$(python3 - "$receipt" "$active_pointer" "$releases_dir" "$runtime_home" "$expected_sha" <<'PY'
...
assert_active_runtime_unchanged() {
  if ! python3 - "$active_pointer" "$active_pointer_sha256" "$base_sha" <<'PY'
import hashlib, json, os, stat, sys

path, expected_digest, expected_sha = sys.argv[1:]
try:
    info = os.lstat(path)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise ValueError("active runtime pointer is not a regular file")
    ...
    raise SystemExit(1)
if actual_digest != expected_digest or value.get("sourceSha") != expected_sha:
    raise SystemExit(1)
PY
  then
    printf '%s\n' 'active runtime changed after candidate preparation' >&2
    exit 75
  fi
}
...
/bin/sh "$gateway_env_wrapper" "$gateway_env_file" \
  /usr/bin/env \
