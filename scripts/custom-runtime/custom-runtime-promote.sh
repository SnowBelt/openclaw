...
usage() {
  printf '%s\n' \
    ...
    '       custom-runtime-promote.sh --lease-acquire|--lease-heartbeat|--lease-authorize-promotion|--lease-release|--lease-recover-expired --active-sha SHA --candidate-sha SHA --owner ID --operation-class release-certification --approval-id ID --operation-id ID --invocation-id ID [--ttl-seconds 3600]' \
...
if [ -n "$lease_action" ]; then
  ...
  custom_runtime_certification_lease "$lease_action" "$runtime_home" \
    ...
    "$lease_operation_id" "$lease_invocation_id" "$lease_reason"
  exit 0
fi
[ -z "$lease_active_sha" ] && [ -z "$lease_candidate_sha" ] && \
  [ -z "$lease_owner" ] && [ -z "$lease_operation_class" ] && \
