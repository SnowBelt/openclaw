...
if [ -e "$target" ]; then
  ...
  target_git_common_parent=$(dirname "$target_git_common_dir")
  target_git_common_name=$(basename "$target_git_common_dir")
  [ -d "$target_git_common_parent" ] && [ -d "$target_git_common_dir" ] || {
    printf '%s\n' 'custom runtime source migration blocked: target Git object store is unavailable' >&2
    exit 64
  }
  target_git_common_parent=$(cd "$target_git_common_parent" && pwd -P)
  target_git_common_dir="$target_git_common_parent/$target_git_common_name"
  case "$target_git_common_dir" in
...
target_git_common_dir=$(cd "$(dirname "$target_git_common_dir")" && pwd -P)/$(basename "$target_git_common_dir")
case "$target_git_common_dir" in
  "$durable_source_root"|"$durable_source_root"/*) ;;
  *)
    printf '{"at":"%s","result":"failed","stage":"target_git_common_dir"}\n' "$timestamp" > "$receipt"
...
if ! python3 - "$pointer" "$target" "$target_git_common_dir" "$remote_url" "$remote_ref" "$source_sha" "$verified_at" <<'PY'
import json
import os
import sys

path, source_repo, git_common_dir, remote_url, remote_ref, remote_sha, migrated_at = sys.argv[1:]
