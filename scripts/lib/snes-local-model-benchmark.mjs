...
export function createSnesLocalModelBenchmarkReport(options = {}) {
  ...
  return {
    ...
    installedModelRefs: [...installed].toSorted((left, right) => left.localeCompare(right)),
...
function localGlmCompletionPayload({ maxOutputTokens = 32, modelId }) {
  return JSON.stringify({
    max_tokens: Math.max(1, Math.trunc(maxOutputTokens)),
...
export function scoreSnesOutputBenchmarkResponse({
  ...
  breakdown.gpt55Judge = Math.max(0, Math.min(10, Math.round(judgeScore ?? 0)));
...
function callLocalGlmModel({ maxOutputTokens, prompt, spawn = spawnSync, timeoutSeconds }) {
  const baseUrl = (process.env.OPENCLAW_LOCAL_GLM52_BASE_URL ?? "http://127.0.0.1:28080").replace(
    /\/+$/u,
    "",
  );
  const payload = JSON.stringify({
  ...
  return {
    blocker:
      result.error || result.status !== 0
        ? (result.error?.message ?? result.stderr ?? "GPT 5.5 judge failed")
