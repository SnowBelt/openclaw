...
export function parseStanskiProductionArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--artifact-dir") {
    ...
    } else if (arg === "--allow-local-glm-restart") {
      args.allowLocalGlmRestart = true;
    } else if (arg === "--base-url") {
      args.baseUrl = argv[++index] ?? args.baseUrl;
    } else if (arg === "--json") {
      args.json = true;
    } else if (arg === "--max-milestones") {
      args.maxMilestones = Number(argv[++index] ?? 1);
      ...
      args.rootDir = argv[++index] ?? args.rootDir;
    } else if (arg === "--skip-smoke") {
      args.runSmoke = false;
    } else if (arg === "--timeout") {
      args.timeoutSeconds = Number(argv[++index] ?? args.timeoutSeconds);
    } else if (arg === "--until") {
      args.until = argv[++index] ?? args.until;
    } else if (STANSKI_PRODUCTION_MODES.has(arg)) {
      args.mode = arg;
...
export function splitNextMilestone(options = {}, snapshot = null, deps = {}) {
  ...
  if (firstChild) {
    current.state.currentMilestoneId = firstChild.id;
  }
  if (current.state.blockedMilestone?.id === milestone.id) {
    current.state.blockedMilestone = null;
  }
...
export function resolveMaxOutputTokensForMilestone(options = {}, milestone = {}) {
  ...
  if (milestone.patchSchema === "levelPatch") {
    return 2_400;
  }
  if (milestone.patchSchema === "mechanicPatch") {
    return 1_800;
  }
  if (milestone.patchSchema === "releasePatch") {
    return 1_600;
  }
...
export function validateGlmMilestonePatch(patch, milestone) {
  ...
  if (patch.localGlmOnly !== true) {
    throw new Error("patch.localGlmOnly must be true");
  }
  if (patch.hostedGlmUsed !== false) {
    throw new Error("patch.hostedGlmUsed must be false");
  }
  ...
  if (hasRawCode(patch)) {
    throw new Error("patch contains raw HTML or JavaScript");
  }
...
export async function askLocalGlmForMilestonePatch(
  ...
  const prompt = createGlmMilestonePrompt(
    retryReason
      ? {
          ...packet,
          retryInstruction: `Previous attempt failed: ${String(retryReason)}. Return smaller strict JSON.`,
...
function policySummary(policy = STANSKI_PRODUCTION_POLICY) {
  return {
    localGlmOnly: policy.localGlmOnly,
    hostedGlmAllowed: policy.hostedGlmAllowed,
    routineGpt55Allowed: policy.routineGpt55Allowed,
    targetHumanVisualGrade: policy.targetHumanVisualGrade ?? 100,
    publishRequiresExecutableQa: policy.publishRequiresExecutableQa,
    publishRequiresRemoteProof: policy.publishRequiresRemoteProof,
  };
}
...
function lockIsActive(lock, paths, deps = {}, policy = STANSKI_PRODUCTION_POLICY) {
  ...
  const staleMs = (policy.staleLockMinutes ?? 20) * 60_000;
  const heartbeatMs = Date.parse(lock.heartbeatAt ?? lock.startedAt ?? "");
  if (Number.isFinite(heartbeatMs) && Date.now() - heartbeatMs > staleMs) {
    return false;
  }
  try {
  ...
  if (options.mode === "auto") {
    ...
    try {
      for (let index = 0; index < Math.max(0, options.maxMilestones ?? 40); index += 1) {
        ...
        if (!selectNextMilestone(snapshot.backlog, snapshot.state)) {
          break;
        }
      }
    } finally {
