...
function stanskiClevelandLandmarks() {
  return STANSKI_CLEVELAND_LANDMARKS.map((landmark) => Object.assign({}, landmark));
}
...
export function detectTool(toolId) {
  ...
  for (const candidate of candidatesForTool(toolId)) {
    if (candidate && existsSync(candidate)) {
      ...
      if (!isUsableResolvedTool(toolId, resolved)) {
        continue;
      }
      return {
        available: true,
...
function resolveToolPath(toolId, candidate) {
  ...
  if (toolId === "pvsneslib") {
    ...
    if (existsSync(path.join(nested, "devkitsnes"))) {
      return nested;
    }
    return candidate;
  }
...
function brewInfo(kind, name) {
  ...
  if (!brew) {
    return { ok: false, blocker: "Homebrew is not available." };
  }
...
function extractArchive(archivePath, outputDir) {
  ...
  if (lower.endsWith(".dmg")) {
    ...
    try {
      ...
      if (!appBundle) {
        return { ok: false, blocker: `No .app bundle found in ${archivePath}.` };
      }
...
function installFromGithubRelease(toolId) {
  const def = TOOL_DEFINITIONS[toolId];
  if (!def.github) {
    return { ok: false, blocker: `${def.label} has no GitHub release recipe.` };
  }
...
function buildFromSource(toolId) {
  const def = TOOL_DEFINITIONS[toolId];
  if (!def.source) {
    return { ok: false, blocker: `${def.label} has no source build recipe.` };
  }
  ...
  if (existsSync(path.join(sourceRoot, "CMakeLists.txt"))) {
    ...
    if (configured.ok) {
      buildAttempts.push(["cmake", ["--build", buildDir, "--config", "Release"]]);
    }
  }
  ...
  for (const [command, args] of buildAttempts) {
    ...
    if (!built.ok) {
      failures.push(
        `${String(command)} ${args.map((arg) => String(arg)).join(" ")}: ${built.stderr || String(built.error) || built.stdout}`,
      );
      continue;
    }
...
function visualProofKindForAsset(type) {
  if (type === "tileset") {
    return "tileset-atlas";
  }
  if (type === "background-layer") {
...
function compiledDimensionsForType(type) {
  ...
  if (type === "tileset") {
    return { width: 192, height: 128, tileCount: 96 };
  }
  if (type === "background-layer") {
    return { width: 256, height: 144, layerCount: 5 };
  }
...
function writeCompiledAssetPng(outputPath, asset, manifest) {
  ...
  return writeRgbPng(outputPath, dimensions.width, dimensions.height, (x, y) => {
    ...
    if (outline || (asset.type === "tileset" && grid)) {
      return ramp[0];
    }
...
function normalizeRequestedAssetId(assetId) {
  ...
  if (normalized === "enemy" || normalized === "foe") {
    return "enemy-sprite";
  }
...
function drawClevelandSkyline(set, width, height, { includeForeground = true } = {}) {
  ...
  for (let x = 0; x < width; x += 9) {
    drawLine(set, x, lakeY + 7, x + 6, lakeY + 7, 15);
  }
  ...
  if (includeForeground) {
    ...
    for (let x = 4; x < width; x += 24) {
      drawRect(set, x, height - 13, x + 10, height - 11, 14);
    }
...
function drawProductionTile(set, ox, oy, tile) {
  ...
  if (family === 0) {
    ...
    for (let y = oy + 3; y < oy + 15; y += 5) {
      drawLine(set, ox, y, ox + 15, y, 7);
    }
    for (let x = ox + 4; x < ox + 16; x += 6) {
      drawLine(set, x, oy, x - 2, oy + 15, 7);
    }
  } else if (family === 2) {
...
function drawToddFrame(set, originX, originY, frameIndex) {
  ...
  if (pose === 4 || pose === 5) {
    drawEllipse(set, originX + 14, originY + 14, 3, 2, 14);
  }
  if (pose === 8) {
  ...
  if (asset.type === "character-sprite") {
    return writeIndexedStylePng(outputPath, dims.width, dims.height, palette, (set) => {
      for (let i = 0; i < 40; i += 1) {
        drawToddFrame(set, (i % 10) * 16, Math.floor(i / 10) * 32, i);
      }
    });
  }
  ...
  const manifestAssets =
    sourcePackRecords.length > 0
      ...
      : (manifest.assets || []).flatMap((asset) => {
          if (asset.type !== "background-layer" || options.assetId) {
            return [asset];
          }
...
function blitRgbImage(
  ...
  for (let y = 0; y < sh; y += 1) {
    for (let x = 0; x < sw; x += 1) {
      ...
      if (transparent && isTransparentAssetRgb(rgb)) {
        continue;
      }
...
function visualQualityBlockers({
  ...
  if (grades?.tileset < 50) {
    blockers.push(`Tileset grade is ${grades.tileset}/100.`);
  }
...
function runtimeUsageForAssetType(type) {
  if (type === "character-sprite") {
    return "player metasprite/OAM frames";
  }
  if (type === "enemy-sprite") {
    return "enemy metasprite/OAM frames";
  }
  if (type === "item-sprite") {
    return "collectible, power-up, projectile, and ending sprites";
  }
  if (type === "tileset") {
    return "foreground collision and level tilemap";
  }
  if (type === "background-layer") {
    return "background/parallax layer";
  }
  ...
  return runtimeAssetsUsed.every(
    (asset) =>
      ...
      asset.convertedOutputs.some(
        (output) => output.path.endsWith("tiles.4bpp") && existsSync(output.path),
      ),
  );
}
...
function encodeSnes4bppTile(rows) {
  ...
  for (const row of rows) {
    ...
    if (cells.length !== 8) {
      throw new Error(`SNES 4bpp tile row must be 8 pixels: ${cells}`);
    }
    let p0 = 0;
    let p1 = 0;
    let p2 = 0;
...
function projectDisplayName(projectId) {
  if (projectId === "comet-fox-mvp") {
    return "Comet Fox MVP";
  }
  if (projectId === STANSKI_PROJECT_ID) {
    return "Stanski's World";
  }
  if (/stanski/iu.test(projectId)) {
    return "Stanski's World Canary";
  }
  return projectId
    .split("-")
    .filter(Boolean)
...
function createStanskiWorldProjectPackage(options = {}) {
  ...
  const imageRecords = STANSKI_REFERENCE_ASSETS.map((asset) => {
    ...
    const convertedAvailable = converted.available;
...
function createMinimalProjectPackage(projectId) {
  const safeId = sanitizeProjectId(projectId);
  if (safeId === STANSKI_PROJECT_ID) {
    return createStanskiWorldProjectPackage();
  }
  const generatedAt = nowIso();
  const projectName = projectDisplayName(safeId);
  const project = {
...
function writeStanskiCanonSummary(projectPackage, options = {}) {
  if (projectPackage?.projectId !== STANSKI_PROJECT_ID) {
    return;
  }
  const root = stanskiProjectReferencesRoot(options);
  ensureDir(root);
...
function ensureGenericProductionFiles(projectId, projectPackage, options = {}) {
  ...
  if (!existsSync(memoryCardsPath)) {
    writeJson(memoryCardsPath, []);
  }
  ...
  if (paths.projectId === STANSKI_PROJECT_ID) {
    writeStanskiCanonSummary(projectPackage, options);
  }
}

function stanskiPackageNeedsCanonRefresh(existing) {
  if (existing?.source !== "stanski-production") {
    return true;
  }
  ...
  for (const asset of STANSKI_REFERENCE_ASSETS) {
    ...
    if (actualReference.status !== reference?.status) {
      return true;
    }
    if (actualReference.sha256 && actualReference.sha256 !== reference?.sha256) {
      return true;
    }
    const converted = snesImageAssetReceipt(asset.assetId);
    if (converted.available) {
      if (record?.sourcePath !== converted.output.path) {
        return true;
      }
      if (record?.sourceHash !== converted.output.sha256) {
        return true;
      }
      if (record?.visualMaturity !== "draft-generated") {
        return true;
      }
      continue;
    }
    if (reference?.status === "preserved" && record?.conversionStatus !== "blocked") {
      return true;
    }
  }
  return false;
...
function visualRejectionEvidence(projectPackage) {
  ...
  if (!visualRejection || visualRejection.status !== "rejected") {
    return null;
  }
  return {
    format: "openclaw-snes-human-visual-rejection-v1",
...
function visualProofEvidence(projectPackage, latestVisualProof = null) {
  ...
  for (const record of records) {
    for (const proof of Array.isArray(record?.visualProof) ? record.visualProof : []) {
      if (!isProductionVisualProof(proof) || !proof?.sha256) {
        continue;
      }
  ...
  if (!rom) {
    ...
    while (queue.length) {
      ...
      for (const entry of readdirSync(current, { withFileTypes: true })) {
        ...
        if (entry.isFile() && /\.(sfc|smc)$/iu.test(entry.name)) {
...
function latestRomReceipt(artifactRoot) {
  ...
  const candidates = readdirSync(artifactRoot)
    ...
    .filter(existsSync)
    .toSorted();
  for (const receiptPath of candidates.toReversed()) {
    const receipt = readJsonIfExists(receiptPath);
...
function detectEmulatorExecutables() {
  ...
  for (const id of ["mesen", "bsnes"]) {
    ...
    if (tool.available) {
      candidates.push({ id, path: tool.path });
    }
  }
  const snes9x =
  ...
  if (snes9x) {
    candidates.push({ id: "snes9x", path: snes9x });
  }
  return candidates;
}
...
export function emulatorSmoke(options = {}) {
  ...
  if (launch.appBundle) {
    report.launch.appBundle = launch.appBundle;
  }
...
function classifyGeneratedSource(sourceText, sourcePath = "") {
  ...
  if (!hasSpriteRuntime) {
    blockers.push("Generated ROM has no OAM/metasprite runtime evidence.");
  }
  if (!hasTilemapRuntime) {
    blockers.push("Generated ROM has no real BG/tilemap runtime evidence.");
  }
  if (!hasAudioRuntime) {
    blockers.push("Generated ROM has no SNES audio runtime evidence.");
  }
  return blockers;
}
...
function engineRomProductionBlockers(receipt) {
  const blockers = [];
  if (!receipt) {
    return ["No engine ROM receipt exists."];
  }
...
function productionAssetBlockers(assetRecords) {
  ...
  for (const type of REQUIRED_PROJECT_ASSET_TYPES) {
    if (!byType.has(type)) {
      blockers.push(`Missing required production asset type: ${type}.`);
    }
  }
  return [...new Set(blockers)];
}
...
function findRomFile(root) {
  ...
  return (
    candidates.toSorted((left, right) => statSync(right).mtimeMs - statSync(left).mtimeMs)[0] ||
    null
  );
}
...
`,
  ...
  if (existsSync(legacyHelloWorldPath)) {
    rmSync(legacyHelloWorldPath);
  }
  if (existsSync(legacyMousePath)) {
    rmSync(legacyMousePath);
  }
...
export function reconcileProductionState(options = {}) {
  ...
  if (receiptPass(latest.artCompile)) {
    built.push("project-art-compile");
  }
  if (receiptPass(latest.conversion)) {
    built.push("project-conversion");
  }
  if (receiptPass(latest.audioCompile)) {
    built.push("project-audio-compile");
  }
  if (receiptPass(latest.artCompile) && receiptPass(latest.conversion)) {
    completed.add("SW-B1-M9");
  }
  if (receiptPass(latest.visualProof)) {
    visualProofed.push("project-visual-proof");
  }
  if (receiptPass(latest.runtimeAssetTruth)) {
    visualProofed.push("project-runtime-asset-truth");
  } else if (latest.runtimeAssetTruth?.blockers?.length) {
    blockers.push(...latest.runtimeAssetTruth.blockers);
  }
  if (receiptPass(latest.browserPlaytest)) {
    visualProofed.push("project-browser-playtest");
  }
  if (receiptPass(latest.visualReviewPack)) {
    visualProofed.push("project-visual-review-pack");
  }
  ...
  if (receiptPass(latest.fxpakCopy)) {
    ...
    if (receiptPass(latest.fxpakTransferPackage)) {
      built.push("fxpak-transfer-package");
    }
  ...
  const completedMilestones = [...completed].toSorted((left, right) => left.localeCompare(right));
...
export function projectVisualReviewPack(options = {}) {
  ...
  const reviewedArtifactHashes = reviewArtifacts
...
export function projectEngineEmulator(options = {}) {
  ...
  for (const candidate of emulators) {
    ...
    if (attempt.appBundle) {
      summary.appBundle = attempt.appBundle;
    }
    if (attempt.launchStrategy) {
      summary.launchStrategy = attempt.launchStrategy;
    }
    if (attempt.fallbackFromOpen) {
      summary.fallbackFromOpen = attempt.fallbackFromOpen;
    }
...
function parseVolumeFilesystem(volumePath) {
  if (process.env.OPENCLAW_SNES_FXPAK_FILESYSTEM) {
    return process.env.OPENCLAW_SNES_FXPAK_FILESYSTEM;
  }
  if (process.env.OPENCLAW_SNES_STUDIO_FXPAK_FS) {
    return process.env.OPENCLAW_SNES_STUDIO_FXPAK_FS;
  }
  const diskutil = run("diskutil", ["info", volumePath], { timeoutMs: 20_000 });
  const text = `${diskutil.stdout}\n${diskutil.stderr}`;
  if (/FAT32|MS-DOS FAT32|File System Personality:\s*MS-DOS/iu.test(text)) {
    return "FAT32";
  }
  if (/exFAT/iu.test(text)) {
    return "exFAT";
  }
  if (/APFS/iu.test(text)) {
    return "APFS";
  }
  if (/HFS\+|Mac OS Extended/iu.test(text)) {
    return "HFS+";
  }
  return "unknown";
}
...
function validateFxpakVolume(volumePath, options = {}) {
  ...
  if (!resolved) {
    blockers.push("No mounted FXPAK/SD2SNES FAT32 media was detected.");
  }
...
export function fxpakDryRun(options = {}) {
  ...
  if (report.blockers.length > 0) {
    if (!volume) {
      report.blockers = ["no mounted FXPAK/SD2SNES FAT32 media"];
    }
    return report;
  }
  const romReceipt = rom.receipt;
