...
export function createGameProject({
  ...
  for (const milestoneId of pccNext({ project, root }).parallelBatches?.[0] ?? []) {
    ...
    if (packet.status === "pass") {
      workerPackets.push(packet.packet);
    }
  }
...
export function validateAssetIntentContract(intent) {
  ...
  if (!Number.isInteger(frameCount) || frameCount < 1) {
    errors.push("invalid-frameCount");
  }
  ...
  if (hasNamedGameReference(intent)) {
    errors.push("project-specific-name-detected");
  }
  return {
...
export function validateAssetPipelineContract(pipeline) {
  const errors = [];
  if (pipeline?.format !== "openclaw-snes-asset-pipeline-v1") {
    errors.push("invalid-format");
  }
  if (pipeline?.projectSpecific !== false) {
    errors.push("projectSpecific-must-be-false");
  }
  if (pipeline?.hostedGlmUsed !== false) {
    errors.push("hosted-glm-forbidden");
  }
  if (pipeline?.commercialMaterialUsed !== false) {
    errors.push("commercial-material-forbidden");
  }
  if (pipeline?.fxpakWritePerformed !== false) {
    errors.push("fxpak-write-forbidden");
  }
  ...
  if (intentValidation.status !== "pass") {
    for (const error of intentValidation.errors ?? []) {
      errors.push(`assetIntent:${error}`);
    }
  }
  ...
  if (stages.contactSheet?.required !== true) {
    errors.push("contactSheet-required");
  }
  ...
  if (hasNamedGameReference(pipeline)) {
    errors.push("project-specific-name-detected");
  }
  return {
...
export function validateLevelContract(level) {
  const errors = [];
  if (level?.format !== "openclaw-snes-level-contract-v1") {
    errors.push("invalid-format");
  }
  if (level?.projectSpecific !== false) {
    errors.push("projectSpecific-must-be-false");
  }
  ...
  if (level?.template && !GAME_TEMPLATES[level.template]) {
  ...
  for (const type of ["enemy", "item", "checkpoint"]) {
    if (!objectTypes.has(type)) {
      errors.push(`missing-object-type:${type}`);
    }
  }
  if (level?.runtimeProofRequired !== true) {
    errors.push("runtimeProofRequired-must-be-true");
  }
  if (level?.hostedGlmUsed !== false) {
    errors.push("hosted-glm-forbidden");
  }
  if (level?.commercialMaterialUsed !== false) {
    errors.push("commercial-material-forbidden");
  }
  if (level?.fxpakWritePerformed !== false) {
    errors.push("fxpak-write-forbidden");
  }
  if (hasNamedGameReference(level)) {
    errors.push("project-specific-name-detected");
  }
  return {
    ...
    ok: errors.length === 0,
    errors,
    objectTypes: [...objectTypes].toSorted((left, right) => left.localeCompare(right)),
    projectSpecific: false,
    hostedGlmUsed: false,
    gpt55Used: false,
    commercialMaterialUsed: false,
...
export function validateHardwareProofPlanTemplate(plan) {
  ...
  if (plan?.projectSpecific !== false) {
    errors.push("projectSpecific-must-be-false");
  }
  if (plan?.proofSeparationRequired !== true) {
    errors.push("proof-separation-required");
  }
  ...
  for (const name of [
    ...
    if (!surfaces[name]) {
      errors.push(`missing-proof-surface:${name}`);
    }
  }
  if (surfaces.fxpakCopyProof?.status === "pass") {
    errors.push("fxpak-copy-cannot-auto-pass");
  }
  if (surfaces.originalHardwareProof?.status === "pass") {
    errors.push("original-hardware-cannot-auto-pass");
  }
  if (surfaces.fxpakCopyProof?.manual !== true) {
    errors.push("fxpak-copy-must-be-manual");
  }
  ...
  if (budget.frameTargetMs !== 16.64) {
    errors.push("invalid-frame-target");
  }
  if (budget.gameplayDmaBytesPerFrameMax !== 4096) {
    errors.push("invalid-dma-budget");
  }
  if (budget.vramAllocatedBytesMax !== 61440) {
    errors.push("invalid-vram-budget");
  }
  if (budget.oamActiveEntriesMax !== 96) {
    errors.push("invalid-oam-budget");
  }
  if (budget.scanlineSpritePolicyMax !== 28) {
    errors.push("invalid-scanline-budget");
  }
  if (budget.aramActiveBytesMax !== 57344) {
    errors.push("invalid-aram-budget");
  }
  if (plan?.hostedGlmUsed !== false) {
    errors.push("hosted-glm-forbidden");
  }
  if (plan?.commercialMaterialUsed !== false) {
    errors.push("commercial-material-forbidden");
  }
  if (plan?.fxpakWritePerformed !== false) {
    errors.push("fxpak-write-forbidden");
  }
  if (plan?.removableMediaWritePerformed !== false) {
    errors.push("removable-write-forbidden");
  }
  if (hasNamedGameReference(plan)) {
    errors.push("project-specific-name-detected");
  }
  return {
...
function validateGenericMvpReferenceProofs(referenceProofs) {
  ...
  for (const [name, reference] of Object.entries(referenceProofs)) {
    ...
    if (receipt?.status !== "pass") {
      blockers.push(`${name}:status-not-pass`);
    }
    if (receipt?.projectSpecific !== false) {
      blockers.push(`${name}:projectSpecific-not-false`);
    }
    if (receipt?.hostedGlmUsed !== false) {
      blockers.push(`${name}:hosted-glm-not-false`);
    }
    if (receipt?.gpt55Used !== false) {
      blockers.push(`${name}:gpt55-not-false`);
    }
    if (hasNamedGameReference(receipt)) {
      blockers.push(`${name}:project-specific-name-detected`);
    }
  }
  const route = referenceProofs.route?.receipt;
  if (!route?.rom?.sha256) {
    blockers.push("route:missing-rom-sha256");
  }
  if (route?.superfamicheck?.ok !== true) {
    blockers.push("route:superfamicheck-not-pass");
  }
  ...
  if (emulator?.emulatorProof?.status !== "pass") {
    blockers.push("emulator:proof-not-pass");
  }
  if (!emulator?.emulatorProof?.screenshotHash) {
    blockers.push("emulator:missing-screenshot-hash");
  }
  ...
  if (budget?.percentComplete !== 100) {
    blockers.push("budget:not-100-percent");
  }
  ...
  if (packageDryRun?.package?.status !== "pass") {
    blockers.push("package:dry-run-not-pass");
  }
  ...
  if (!projectGenerator?.rom?.sha256) {
    blockers.push("project-generator:missing-rom-sha256");
  }
...
export function parseSnesTeamArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (arg === "--asset-pipeline") {
      args.assetPipelinePath = argv[++index];
    } else if (arg === "--level") {
      args.levelPath = argv[++index];
    } else if (arg === "--worker-output") {
      args.workerOutputPath = argv[++index];
    } else if (arg === "--hardware-plan") {
      args.hardwarePlanPath = argv[++index];
    } else if (arg === "--reference-root") {
      args.referenceRoot = argv[++index];
...
export function runSnesTeam(args) {
  ...
  try {
    ...
    if (mode === "level-validate") {
      return validateLevelContract(readJson(args.levelPath));
    }
