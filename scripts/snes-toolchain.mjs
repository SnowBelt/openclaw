...
try {
  ...
  if (args.json) {
    ...
    if (report.artifacts?.receiptPath) {
      console.log(`Receipt: ${report.artifacts.receiptPath}`);
    }
    if (report.blockers?.length) {
      console.log(`Blocker: ${report.blockers[0]}`);
    }
    if (report.probe?.blockers?.length) {
      console.log(`Blocker: ${report.probe.blockers[0]}`);
    }
  }
  const proofMode = [
    "conversion-smoke",
