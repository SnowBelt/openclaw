...
the local-first Gemma primary and Qwen chatfix fallback policy. `--strategic`
...
When the Gemma primary is missing but the chatfix fallback is responsive, the
...
- `--review-model` defaults to `ollama/openclaw-control-gemma4-31b-q8:latest`
...
The Gemma primary and Qwen chatfix fallback are tuned for schema discipline with
...
If chatfix succeeds after a blocked Gemma primary, the result shows
...
quality. When the Gemma primary is blocked but chatfix is reachable and returns
