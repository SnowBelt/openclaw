...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - **Load earlier messages** pages backward without replacing the visible transcript. Automatic/daily/compaction session rotations that retain the same usage lineage are read as one chronological conversation, while an explicit **New Session** remains a deliberate new conversation.
    ...
    - Leave the recommended Control Director selected for normal work: it plans and delegates through configured local-first routing. Specialist agents remain available in an **Advanced** group for direct operator use, but they are not required just to pick a model. Model choices are grouped by local/self-hosted, subscription, and metered API route; reasoning and speed controls are limited to capabilities reported for the selected model. Metered API work stays subject to the configured routing and spend policy.
    - The **Project** pill attaches Chat to the same project record used by Project Command Center (PCC). PCC owns milestones, permissions, evidence, and automation; Chat uses that project ID to organize and route conversation context. It does not create a parallel project plan.
    - **Pursue Goal** is the durable execution control for the active Chat session. Pause, resume, retry, edit, and stop act on the canonical Task Flow. A blocked goal appears under **Needs attention** with an owner and next action; an active goal never appears beside a contradictory "Nothing running" label.
    - The model-facing `/goal` session objective remains an internal token-budget aid for compatible runtimes. Chat does not render it as a second operator goal. **Pursue Goal** is the only user-facing execution control, and Resume/Retry immediately continue work instead of only changing a status label.
    - Collapsed activity rows summarize observable work as **OpenClaw is working** or **What OpenClaw did** and expand to tool details. They intentionally do not expose hidden chain-of-thought. **Truth & Completion** appears only when the Control Director blocked an unsupported completion claim, stays collapsed in the transcript, and explains why it stopped plus the safe next action; healthy/no-diagnostic sessions do not consume Chat space.
    - Chat is the conversation and execution lane. PCC is the portfolio, milestone, permission, proof, and automation lane. Both read the same project and Task Flow identities so status remains consistent without duplicating ownership.
