...
<AccordionGroup>
  ...
  <Accordion title="Cron, skills, nodes, exec approvals">
    ...
    - Agents -> Self-Improvement: Action Queue counts, owner/SLA/proof/closure chips, operational health cards, production readiness, retention maintenance dry-runs, Improvement Intelligence counts/top opportunities, grouped recommendation cards, daily scorecards, proposal queue, Memory/Skill Curator Queue with pending-mode accept/link/promote/reject controls, sanitized audit ledger, routing, analysis/LLM fallback status, required evidence, safety state, proof requirements, safe assignment/claim/proof/resolve/dismiss controls, manual scan, and bounded analysis run controls.
    - Agents -> Live Agent Workspace groups agents into canonical workspace rooms. The selected-agent detail card includes a Workspace room picker that saves `roomId` through `agents.update`, so mapped and newly discovered agents can be reassigned without editing raw config. For the Control Director, the card separates the configured primary model from the last-run model and rollback model so historical Qwen2.5 fallback use is not shown as the current Gemma primary.
    - Agents -> Self-Improvement is a separate recommendation-only Governor surface. Control Director readiness can appear there as project-health evidence, but the Control Director is not the Self-Improvement Governor and does not own Governor scan, routing, closure, or model-review policy.
