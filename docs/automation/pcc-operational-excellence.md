...
The desired-state inventory is `config/custom-runtime-capabilities.json`. It covers dashboard surfaces, required plugins, PCC workflows, Control Director truth gates, local-first model intelligence, and the update-safety control plane. Every capability has a stable ID and one or more required runtime paths. Revision 5 additionally requires every tracked `scripts/custom-runtime/` control-plane file to have an explicit capability owner. The v2 preservation contract also binds required criticality, preserve-or-block migration, immutable-pointer rollback, exact-active-SHA merge strategy, register-verify-and-block Dashboard policy, explicit exact-candidate approval, the canonical update-survival proof, ordered verification commands, and the checked addition-standards registry. The immutable runtime pointer binds the capability manifest hash and the cumulative required capability IDs. Candidates may add capabilities, but cannot silently remove or repurpose an active capability or required path.

The current inventory contains 31 preserved capabilities. It includes all seven app dashboards, both required plugins, PCC project management and operational excellence, Control Director truth gates, local-first model intelligence, the complete Codex-style Chat stack, mobile PCC control, PCC-to-Chat synchronization, Chat UX cleanup, the cumulative Codex-plus-apps dashboard, Operations Room, update-safe customizations, and the Self-Improvement Governor runtime. Each preserved browser surface also has a named `pnpm ui:smoke:*` command so a future update cannot pass by keeping source files while silently removing the executable proof path.
...
`custom-runtime-stage.sh` runs the candidate against copied config and state on a private port; it does not modify the active pointer. Managed promotion installs and loads the weekly prepare-only broker and runtime recovery guard from the promoted release. The broker prepares an exact-parent candidate, proves monotonic capability/path preservation, runs the manifest-defined gates, and stops at `ready_for_approval`. `custom-runtime-update-approve.sh` verifies the bound preservation-proof digest and promotes only that exact candidate after explicit approval. `custom-runtime-promote.sh` updates the pointer and managed services only after staging, and restores the prior pointer, Gateway plist, environment file, and loaded state of both auxiliary LaunchAgents if a gate fails. PCC and Control Director readiness report the broker and guard separately so source-only or unloaded control files cannot satisfy update-safety acceptance. See [Custom Runtime Update Safety](/automation/custom-runtime-update-safety) for the complete operator flow.
