...
`custom-runtime-stage.sh` runs the candidate against copied config and state on a private port; it does not modify the active pointer. The weekly broker prepares an exact-parent candidate, proves monotonic capability/path preservation, runs the manifest-defined gates, and stops at `ready_for_approval`. `custom-runtime-update-approve.sh` verifies the bound preservation-proof digest and promotes only that exact candidate after explicit approval. `custom-runtime-promote.sh` updates the pointer and managed service only after staging, and restores the prior pointer, plist, and environment file if bootstrap or health proof fails. See [Custom Runtime Update Safety](/automation/custom-runtime-update-safety) for the complete operator flow.
