...
When certification resumes after a previously promoted exact candidate is already active, reacquiring a lease does not require re-promoting the same release or replacing its rollback registration. The owner first authorizes promotion on the new exact lease, then runs `--lease-adopt-active-promotion` with the same binding. Adoption succeeds only when active and candidate SHAs are identical, the runtime is under the immutable releases root, the immutable runtime stamp matches, and prior promotion plus lifecycle receipts match the active release, SHA, promotion timestamp, and successful operation. It records both receipt digests and advances the lease through the existing owner-authorized lifecycle without changing the runtime, Gateway, configuration, or rollback state.
