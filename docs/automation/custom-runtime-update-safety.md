...
Long-running exact-SHA certification can additionally acquire an expiring lease through `custom-runtime-promote.sh --lease-acquire`. The lease binds the current active SHA, candidate SHA, owner, `release-certification` operation class, creation time, and expiration time under the same promotion lock used by runtime mutation. While it is active, only that candidate can pass promotion; a changed active runtime, competing candidate, malformed lease, or expired lease fails closed without weakening Release Governor verification. Operators must release a live lease with the exact original binding or explicitly recover an expired lease; both paths write a private receipt before removing it.
