...
PCC records the small-team guardrails in the ledger and exposes them in the dashboard: up to six authenticated operators, 100 active projects, two simultaneous Codex planning runs, and up to 200 files or 1 GiB of attachments per project. Additional planning requests wait in a visible, cancellable queue and enter the next available slot automatically. Abandoned upload parts are removed automatically before a new upload begins, while committed attachments remain content-addressed and untouched.
