...
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, OpenClaw fails visibly instead of falling through to another configured model. Control Director selections are preflighted more strictly: a bare id must uniquely match the configured/cataloged model view, and any non-default provider/model must be present in that model view before the session pin is accepted.
