...
export function validateSnesStudioProject(projectInput: SnesStudioProject): SnesValidationIssue[] {
  const project = normalizeSnesStudioProject(projectInput);
...
export function createSnesOpenClawAgentResults(
  ...
  const unresolved = report.gaps.filter((aiGap) => !aiGap.resolved);
  const readiness = buildSnesReadiness(project);
  return blueprint.agentTasks.map((task) => {
    const missingPieces =
      task.role === "hardware-constraint-agent"
        ...
        : unresolved
            .filter((aiGap) => aiGap.severity === "blocker" || aiGap.severity === "warning")
            .map((aiGap) => aiGap.title);
    return {
      taskId: task.id,
      role: task.role,
...
export function createSnesAiProductionRun(
  ...
  if (firstReview.status === "fail") {
    ...
    agentResults.push({
      ...
      risks: [],
      missingPieces: repaired.report.gaps
        .filter((aiGap) => !aiGap.resolved)
        .map((aiGap) => aiGap.title),
