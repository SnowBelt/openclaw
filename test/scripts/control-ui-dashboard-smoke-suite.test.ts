...
describe("control-ui-dashboard-smoke-suite", () => {
  it("runs dashboard smokes and hardware proof with CI artifact directories", () => {
    ...
    expect(calls[1]).toMatchObject({
      args: ["ui:smoke:self-improvement"],
      command: "pnpm",
    });
    expect(calls[1]?.env.OPENCLAW_CONTROL_UI_SELF_IMPROVEMENT_ARTIFACT_DIR).toBe(
    ...
    expect(calls[3]).toMatchObject({
      args: ["ui:smoke:book-writer"],
      command: "pnpm",
    });
    expect(calls[3]?.env.OPENCLAW_CONTROL_UI_BOOK_WRITER_ARTIFACT_DIR).toBe(
      ".artifacts/control-ui-book-writer/ci",
    );
    expect(calls[4]).toMatchObject({
      args: ["ui:smoke:snes-studio"],
      command: "pnpm",
    });
    expect(calls[4]?.env.OPENCLAW_CONTROL_UI_SNES_STUDIO_ARTIFACT_DIR).toBe(
  ...
  it("stops after the first failed smoke step", () => {
    ...
    expect(calls).toEqual(["pnpm ui:smoke:projects", "pnpm ui:smoke:self-improvement"]);
  ...
  it("previews release commands without executing them in dry-run mode", () => {
    ...
    expect(summary.steps).toEqual([
      ...
      {
        command: ["pnpm", "ui:smoke:book-writer"],
        exitCode: 0,
        name: "Book Writer dashboard smoke",
      },
      {
