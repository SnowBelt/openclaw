...
describe("control-director-readiness", () => {
  ...
  it("flags a missing Gemma base model as a critical readiness gap", () => {
    const scorecard = buildControlDirectorReadinessScorecard({
      config: createConfig(),
      ollamaModels: new Map([
    ...
    expect(scorecard.failedCritical).toContain("Gemma base model tag is installed");
    expect(scorecard.nextBuildGap).toContain("Gemma base model tag");
  });
