...
const guardScript = path.resolve("scripts/custom-runtime/custom-runtime-guard.sh");
...
describe("custom runtime lifecycle", () => {
  it("waits for a bootstrapping Gateway before requiring recovery evidence", () => {
    const root = createRoot("openclaw-custom-runtime-guard-startup-");
    ...
    const lifecycleReceipt = fs
      ...
      .find((entry) => entry.startsWith("lifecycle-guard-"));
    expect(lifecycleReceipt).toBeTruthy();
    expect(
      JSON.parse(fs.readFileSync(path.join(runtimeHome, "receipts", lifecycleReceipt!), "utf8")),
    ).toMatchObject({ exitCode: 0, result: "guard-healthy" });
