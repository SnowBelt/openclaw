...
import {
  ...
  evaluateProgramManagerRepositoryContracts,
...
describe("agent role eval harness", () => {
  ...
  it("passes the checked-in Program Manager repository contracts", () => {
    expect(evaluateProgramManagerRepositoryContracts()).toEqual({ ok: true, issues: [] });
  });

  it("prompts live evals to emit exact role signals", () => {
    const contract = AGENT_ROLE_CONTRACT_BY_ID.get("program-manager")!;

    expect(contract.prompt).toContain("Planning and status only");
    expect(contract.prompt).toContain("never claim completion without verification evidence");
    expect(contract.prompt).toContain("Create handoff packets only");
    expect(contract.prompt).toContain("do not use sessions_send");
    expect(contract.prompt).toContain("non-secret telemetry");
    expect(contract.prompt).toContain("Stop immediately after the BLOCK_OR_ESCALATE line");
    expect(contract.prompt).toContain("milestone owner");
  });
  ...
  it("creates a self-contained live eval state that passes static checks", () => {
    ...
    expect(fixture.config.agents.list[0]?.tools).toEqual({
      allow: contract.allowedTools,
  ...
  it("keeps the CI live workflow aligned with local-first eval defaults", () => {
    ...
    expect(liveJob?.if).toContain("workflow_dispatch");
    expect(liveJob?.if).not.toContain("schedule");
    expect(liveJob?.env).not.toHaveProperty("OPENAI_API_KEY");
    expect(liveJob?.env).not.toHaveProperty("OPENCLAW_CODEX_AUTH_JSON");
    expect(liveJob?.steps?.some((step) => step.name === "Hydrate live auth inputs")).toBe(false);
  });
  ...
  it("plans local Ollama startup, memory proof, model pull, and GitHub env handoff", () => {
    ...
    expect(status).toBe(0);
    expect(readFileSync(githubEnv, "utf8")).toMatch(
      new RegExp(
        `^OPENCLAW_AGENT_ROLE_EVAL_RESOLVED_MODEL<<OPENCLAW_ENV_[^\\n]+\\n${DEFAULT_SELF_CONTAINED_LIVE_MODEL}\\nOPENCLAW_ENV_[^\\n]+\\n$`,
      ),
    );
    expect(calls).toEqual([
  ...
  it("fails a Program Manager policy that exposes indirect execution", () => {
    const root = harness.createTempDir("openclaw-agent-eval-");
    const id = "program-manager";
    const workspace = writeAgentWorkspace(root, id, "Program Manager planning and status.");
    const agentDir = writeAgentDir(root, id);
    const result = evaluateAgentStaticContracts(
      baseConfig(root, {
        id,
        ...
        agentDir,
        model: { primary: "ollama/qwen3.5:4b", fallbacks: [] },
        tools: { allow: ["sessions_send"], exec: { security: "allow", ask: "off" } },
      }),
    );
    ...
    expect(result.issues).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ code: "program_manager_exec_policy_unsafe" }),
      ]),
    );
  });
