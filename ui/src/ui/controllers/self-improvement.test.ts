...
describe("loadSelfImprovementRecommendations", () => {
  ...
  it("preserves last-known-good sections when one dashboard read fails", async () => {
    const { state, request } = createState();
    const previousRecommendation = {
      id: "sir_previous",
    } as SelfImprovementState["selfImprovementRecommendations"][number];
    ...
    request.mockImplementation((method: string) => {
      if (method === "selfImprovement.summary") {
        return Promise.resolve({
          groups: [{ id: "sig_new" }],
          scorecard: { activeRecommendations: 2 },
        });
      }
      if (method === "selfImprovement.recommendations.list") {
        return Promise.reject(new Error("recommendation read failed"));
      }
      if (method === "selfImprovement.scorecard") {
        return Promise.resolve({ current: null, scorecards: [] });
      }
      if (method === "selfImprovement.health") {
        return Promise.reject(new Error("health read failed"));
      }
      if (method === "selfImprovement.proposals.list") {
        return Promise.reject(new Error("proposal read failed"));
      }
      if (method === "selfImprovement.curator.list") {
        return Promise.resolve({ proposals: [], total: 0 });
      }
    ...
    expect(state.selfImprovementGroups).toEqual([{ id: "sig_new" }]);
    expect(state.selfImprovementRecommendations).toEqual([previousRecommendation]);
    expect(state.selfImprovementHealth).toBe(previousHealth);
    expect(state.selfImprovementError).toContain("Recommendations:");
    expect(state.selfImprovementError).toContain("Health:");
    expect(state.selfImprovementError).toContain("Proposals:");
  });
