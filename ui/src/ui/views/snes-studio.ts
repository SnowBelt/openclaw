...
function applyPromptToSelectedScreenArea(host: HostUpdate) {
  ...
  markGuidedAiResult(
    ...
    "level",
    summary,
    [changed ?? "selected emulator area"],
...
function renderGameTestPanel(host: HostUpdate) {
  ...
  const emulatorReplayProof: ReturnType<typeof createSnesEmulatorReplayParityProof> | null =
    (() => {
      try {
        return currentEmulatorReplayParityProof();
      } catch {
        return null;
      }
    })();
  ...
  return html`
    <section class="snes-next-step-coach" aria-label="Next best step">
      <div>
        ...
        <h4>
          ${isSelectedSceneThing && focusedObject
...
function renderCanvasActionLoop(host: HostUpdate, scene: SnesStudioProject["scenes"][number]) {
  ...
  const isSelectedSceneThing =
    focusedObject?.id.startsWith(`${scene.id}:`) ||
    focusedObject?.id === `level:${selectedSceneIndex}`;
  const selectedLabel = isSelectedSceneThing && focusedObject ? focusedObject.label : "";
  const loopSteps = [
    {
      label: "1",
      title: "Pick",
