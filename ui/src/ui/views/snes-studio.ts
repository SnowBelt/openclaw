...
function renderGameTestPanel(host: HostUpdate) {
  ...
  let emulatorReplayProof: ReturnType<typeof createSnesEmulatorReplayParityProof> | null;
  try {
  ...
  return html`
    <section class="snes-next-step-coach" aria-label="Next best step">
      <div>
        ...
        <h4>
          ${hasSelectedSceneThing && focusedObject
...
function renderCanvasActionLoop(host: HostUpdate, scene: SnesStudioProject["scenes"][number]) {
  ...
  const hasSelectedSceneThing =
    focusedObject?.id.startsWith(`${scene.id}:`) ||
    focusedObject?.id === `level:${selectedSceneIndex}`;
  const selectedLabel = hasSelectedSceneThing && focusedObject ? focusedObject.label : "";
  const loopSteps = [
    {
      label: "1",
      title: "Pick",
