...
import { chatRunStatusLabel, type ChatRunStatus } from "./run-status.ts";
...
export function buildWorkSurfaceSnapshot(input: BuildWorkSurfaceSnapshotInput): WorkSurfaceItem[] {
  ...
  if (chatRunId) {
    const runLabel =
      chatRunStatusLabel(input.chatRunStatus, input.assistantName) ?? "OpenClaw is working…";
    items.push({
      id: `chat-run:${chatRunId}`,
      kind: "chat_run",
      title: runLabel,
      status: "Working",
      detail: input.chatRunStatus?.detail,
      updatedAt: input.chatRunStatus?.updatedAt,
