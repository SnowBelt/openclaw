...
export type GatewayRuntimeSnapshotReleaseStatus = {
  ...
  usable: boolean;
  active: boolean;
  provenance?: GatewayRuntimeSnapshotProvenance;
};

export type GatewayRuntimeSnapshotProvenance = {
...
function parseSnapshotSchemas(value: unknown): Record<string, number> {
  if (!isRecord(value)) {
    return {};
  }
...
export function readGatewayRuntimeSnapshotProvenance(
  params: {
    snapshotRoot?: string;
    env?: NodeJS.ProcessEnv;
  } = {},
): GatewayRuntimeSnapshotProvenance | null {
  const snapshotRoot = optionalString(
    params.snapshotRoot ?? params.env?.[GATEWAY_RUNTIME_SNAPSHOT_ROOT_ENV_KEY],
  );
  if (!snapshotRoot || !isUsableSnapshotRoot(snapshotRoot)) {
    return null;
  }
  const manifest = readJsonFileIfPresent(path.join(snapshotRoot, "snapshot.json"));
  const source = isRecord(manifest?.source) ? manifest.source : null;
  const manifestVersion = manifest?.version;
  const releaseId = optionalString(manifest?.releaseId) ?? path.basename(snapshotRoot);
  const createdAt = optionalString(manifest?.createdAt);
  if (
  ...
  const packageVersion = optionalString(manifest?.packageVersion);
  const artifactHash = optionalString(manifest?.artifactHash);
  const sourceCommit = optionalString(source?.commit);
  return {
    manifestVersion,
    releaseId,
    createdAt,
    ...(packageVersion ? { packageVersion } : {}),
    ...(artifactHash && /^[0-9a-f]{64}$/u.test(artifactHash) ? { artifactHash } : {}),
    ...(sourceCommit && /^[0-9a-f]{40}$/u.test(sourceCommit) ? { sourceCommit } : {}),
    schemas: parseSnapshotSchemas(manifest?.schemas),
  };
}
...
function resolvePackageRootForEntrypoint(params: {
  ...
  const cwdPackageRoot = resolveOpenClawPackageRootSync({
...
export function getGatewayRuntimeSnapshotStatus(
  ...
  const latestRoot =
    isRecord(latest) && typeof latest.root === "string" && latest.root.trim()
      ...
      : null;
  const activeRoot = optionalString(params.env?.[GATEWAY_RUNTIME_SNAPSHOT_ROOT_ENV_KEY]);
  ...
  const releases = listSnapshotReleases({
    ...
    const provenance = readGatewayRuntimeSnapshotProvenance({ snapshotRoot: release.root });
    return Object.assign({}, release, {
      latest: latestRoot === resolvedRoot,
      protected: protectedRoots.has(resolvedRoot),
      usable: isUsableSnapshotRoot(release.root),
      active: activeRoot ? path.resolve(activeRoot) === resolvedRoot : false,
