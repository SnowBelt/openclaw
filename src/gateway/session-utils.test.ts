...
describe("gateway session utils", () => {
  ...
  test("uses resolved OpenAI GPT context instead of stale oversized persisted session caps", () => {
    ...
    const row = buildGatewaySessionRow({
      ...
      key: "agent:main:main",
      entry: {
        sessionId: "stale-openai-oversized-session",
        modelProvider: "openai",
        model: "gpt-5.5",
  ...
  test("keeps configured OpenAI GPT context caps for session rows", () => {
    const cfg = {
      agents: { defaults: { contextTokens: 64_000, model: { primary: "openai/gpt-5.5" } } },
      models: {
        providers: {
          openai: { models: [{ id: "gpt-5.5", contextTokens: 272_000 }] },
        },
    ...
    const row = buildGatewaySessionRow({
      ...
      key: "agent:main:main",
      entry: {
        sessionId: "capped-openai-session",
        modelProvider: "openai",
        model: "gpt-5.5",
        contextTokens: 64_000,
      } as SessionEntry,
