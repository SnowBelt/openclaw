...
function summarizeProject(ledger: PccLedger, project: PccProject): PccProjectSummary {
  ...
  const counted = milestones.filter((milestone) => !SKIPPED_STATUSES.has(milestone.status));
  const percentComplete = counted.length
    ? Math.round(
        counted.reduce((total, milestone) => total + milestonePercent(ledger, milestone), 0) /
          counted.length,
      )
    : COMPLETE_STATUSES.has(project.status)
      ? 100
      : 0;
  ...
  const nextActions = milestones
    .filter((milestone) => !COMPLETE_STATUSES.has(milestone.status) && !SKIPPED_STATUSES.has(milestone.status))
  ...
  const proofGaps = milestones
    .filter((milestone) => COMPLETE_STATUSES.has(milestone.status) && !hasReceipt(ledger, milestone.id))
    .slice(0, 20)
...
function summarizePortfolio(ledger: PccLedger): PccPortfolioSummary {
  ...
  return {
    ...
    active: ledger.projects.filter((project) => ["active", "in_progress", "reopened"].includes(project.status)).length,
...
function upsertProject(ledger: PccLedger, input: {
  id?: string;
  title: string;
  goal?: string;
  status?: PccStatus;
  ...
  const project: PccProject = {
    ...
    updatedAt: timestamp,
    ...(input.goal !== undefined ? { goal: input.goal } : existing?.goal !== undefined ? { goal: existing.goal } : {}),
    ...(input.owner !== undefined ? { owner: input.owner } : existing?.owner !== undefined ? { owner: existing.owner } : {}),
    ...(input.priority !== undefined
      ? { priority: input.priority }
      : existing?.priority !== undefined
        ? { priority: existing.priority }
        : {}),
    ...(input.phases !== undefined ? { phases: input.phases } : existing?.phases !== undefined ? { phases: existing.phases } : {}),
...
function upsertMilestone(ledger: PccLedger, input: {
  id?: string;
  projectId: string;
  title: string;
  status?: PccStatus;
  ...
  const milestone: PccMilestone = {
    ...
    createdAt: existing?.createdAt ?? timestamp,
    updatedAt: timestamp,
    ...(input.phaseId !== undefined ? { phaseId: input.phaseId } : existing?.phaseId !== undefined ? { phaseId: existing.phaseId } : {}),
    ...(input.owner !== undefined ? { owner: input.owner } : existing?.owner !== undefined ? { owner: existing.owner } : {}),
    ...(input.order !== undefined ? { order: input.order } : existing?.order !== undefined ? { order: existing.order } : {}),
    ...(input.percentComplete !== undefined
      ? { percentComplete: input.percentComplete }
      : existing?.percentComplete !== undefined
        ? { percentComplete: existing.percentComplete }
        : {}),
    ...(input.dependsOn !== undefined ? { dependsOn: input.dependsOn } : existing?.dependsOn !== undefined ? { dependsOn: existing.dependsOn } : {}),
    ...
    ...(input.permissionGrantIds !== undefined
      ...
      : existing?.permissionGrantIds !== undefined
        ...
        : {}),
    ...(input.blocker !== undefined ? { blocker: input.blocker } : existing?.blocker !== undefined ? { blocker: existing.blocker } : {}),
...
export const pccHandlers: GatewayRequestHandlers = {
  ...
  "pcc.milestones.upsert": ({ params, respond }) => {
    ...
    try {
      ...
      if ("error" in result) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, result.error ?? "PCC request failed"));
        return;
      }
      respond(true, result);
    } catch (error) {
    ...
    try {
      const result = withLedger(
        (ledger) => {
          ...
          if (params.permission.milestoneId && !milestoneOrError(ledger, params.permission.milestoneId)) {
          ...
          const permission: PccPermissionGrant = {
            id: existing?.id ?? params.permission.id ?? makeId("permission", params.permission.type),
            ...
            usedCount: status === "used" ? (existing?.usedCount ?? 0) + 1 : (existing?.usedCount ?? 0),
            auditLog,
            createdAt: existing?.createdAt ?? timestamp,
            updatedAt: timestamp,
            ...
            ...(params.permission.grantedBy !== undefined
              ...
              : existing?.grantedBy !== undefined
                ...
                : {}),
            ...(status === "granted" ? { grantedAt: existing?.grantedAt ?? timestamp } : existing?.grantedAt !== undefined ? { grantedAt: existing.grantedAt } : {}),
          };
    ...
    try {
      const result = withLedger(
        (ledger) => {
          ...
          if (params.evidence.milestoneId && !milestoneOrError(ledger, params.evidence.milestoneId)) {
          ...
          const evidence: PccEvidence = {
            ...
            ...(params.evidence.exitCode !== undefined ? { exitCode: params.evidence.exitCode } : {}),
            ...(params.evidence.metadata !== undefined ? { metadata: params.evidence.metadata } : {}),
          };
      ...
      const result = withLedger(
        (ledger) => {
          ...
          const missingEvidence = params.receipt.proofEvidenceIds.filter(
            (id) => !ledger.evidence.some((evidence) => evidence.id === id && evidence.projectId === project.id),
          );
          ...
          const receipt: PccCompletionReceipt = {
            ...
            ...(params.receipt.artifactRefs !== undefined ? { artifactRefs: params.receipt.artifactRefs } : {}),
            ...(params.receipt.doNotRedo !== undefined ? { doNotRedo: params.receipt.doNotRedo } : {}),
            ...(params.receipt.followUpGaps !== undefined ? { followUpGaps: params.receipt.followUpGaps } : {}),
            ...(params.receipt.completedBy !== undefined ? { completedBy: params.receipt.completedBy } : {}),
          };
          ...
          return { receipt, milestone: updatedMilestone, summary: summarizeProject(ledger, project) };
        },
        { write: true },
      );
      if ("error" in result) {
  ...
  "pcc.lastKnownGood.upsert": ({ params, respond }) => {
    if (!validatePccLastKnownGoodUpsertParams(params)) {
      respondInvalid(respond, "pcc.lastKnownGood.upsert", validatePccLastKnownGoodUpsertParams.errors);
      return;
    }
    try {
      const result = withLedger(
        (ledger) => {
          ...
          const existing = params.entry.id
            ...
            : ledger.lastKnownGood.find(
                (entry) => entry.projectId === params.entry.projectId && entry.subsystem === params.entry.subsystem,
              );
          const entry: PccLastKnownGood = {
            ...
            verifiedAt: nowIso(),
            ...(params.entry.evidenceIds !== undefined ? { evidenceIds: params.entry.evidenceIds } : existing?.evidenceIds !== undefined ? { evidenceIds: existing.evidenceIds } : {}),
            ...(params.entry.sha !== undefined ? { sha: params.entry.sha } : existing?.sha !== undefined ? { sha: existing.sha } : {}),
  ...
  "pcc.summary.get": ({ params, respond }) => {
    ...
    try {
      ...
      if (params.projectId) {
        ...
        respond(true, { project: summarizeProject(ledger, project), portfolio: summarizePortfolio(ledger) });
        return;
      }
