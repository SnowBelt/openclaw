...
function fixture() {
  ...
  const backupReceiptPath = path.join(receiptsRoot, "update-backup-20260831T000000Z.json");
  fs.writeFileSync(
    backupReceiptPath,
    `${JSON.stringify({
  ...
  fs.chmodSync(backupReceiptPath, 0o600);
  return {
    homedir,
    pointerPath,
    recordPath,
...
describe("custom runtime update policy", () => {
  ...
  it("reports an available destination without a fresh verified receipt", () => {
    const value = fixture();
    fs.rmSync(path.join(path.dirname(value.pointerPath), "receipts"), { recursive: true });
    ...
    expect(result.preparationStatus).toBe("idle");
    expect(result.preparationReason).toBeNull();
  });

  it("reports a malformed verified receipt as failed", () => {
    const value = fixture();
    const receiptsRoot = path.join(path.dirname(value.pointerPath), "receipts");
    fs.rmSync(path.join(receiptsRoot, "update-backup-20260831T000000Z.json"));
    fs.writeFileSync(path.join(receiptsRoot, "update-backup-20260831T010000Z.json"), "not json\n", {
      mode: 0o600,
    });
  ...
  it("fails closed when the environment backup destination conflicts with its configuration", () => {
    ...
    const result = resolveCustomRuntimeUpdatePolicy({
      ...
      env: {
        ...
        OPENCLAW_CUSTOM_RUNTIME_BACKUP_ROOT: conflictingRoot,
      },
    });
