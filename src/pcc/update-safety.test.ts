...
<string>/bin/sh</string><string>${gatewayEnvWrapperPath}</string><string>${gatewayEnvFilePath}</string><string>${guardExecutablePath}</string>
    ...
    fs.writeFileSync(
      path.join(runtimeHome, "receipts", "update-backup-20260717T000000Z.json"),
      '{"at":"20260717T000000Z","result":"backup-created"}\n',
    );
    const guardReceiptPath = path.join(runtimeHome, "receipts", "guard-verification-current.json");
    const originalGuardPlist = fs.readFileSync(guardPlistPath, "utf8");
    const originalGuardReceipt = fs.readFileSync(guardReceiptPath, "utf8");
    ...
    fs.writeFileSync(
      guardPlistPath,
      originalGuardPlist.replace(gatewayEnvFilePath, path.join(homedir, "unexpected.env")),
    );
    const guardReceipt = JSON.parse(originalGuardReceipt) as Record<string, unknown>;
    guardReceipt.guardPlistSha256 = sha256(guardPlistPath);
    fs.writeFileSync(guardReceiptPath, `${JSON.stringify(guardReceipt)}\n`, { mode: 0o600 });
    const invalidGuard = await readPccUpdateSafety({
      homedir,
      runtimeHome,
      pointerPath,
      schedulerLoaded: true,
    ...
    expect(invalidGuard.status).toBe("attention");
    expect(invalidGuard.issues).toContain("The recovery guard LaunchAgent contract is invalid.");
    fs.writeFileSync(guardPlistPath, originalGuardPlist);
    fs.writeFileSync(guardReceiptPath, originalGuardReceipt, { mode: 0o600 });
