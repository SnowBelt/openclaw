...
function metadataFingerprintFields(metadata: unknown): string[] {
  const source =
    metadata && typeof metadata === "object" && !Array.isArray(metadata)
      ...
      : {};
  return [
    String(source.pccResponsibility ?? ""),
    String(source.pccProofLevel ?? ""),
    String(source.pccExecutionOwner ?? ""),
    String(source.pccProofRequired ?? ""),
  ];
}
  ...
  const subMilestonesChanged = generated.subMilestones.some((generatedSubMilestone) => {
    ...
    const subMetadata =
      ...
      !Array.isArray(existingSubMilestone.metadata)
        ? (existingSubMilestone.metadata as Record<string, unknown>)
        : {};
    return (
