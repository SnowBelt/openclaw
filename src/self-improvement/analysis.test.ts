...
describe("runSelfImprovementAnalysis", () => {
  ...
  it("records deterministic fallback metadata when local model preflight is unavailable", async () => {
    ...
    expect(result).toMatchObject({
      ...
      blockedPrimaryReason:
        "ollama/openclaw-control-gemma4-31b-q8:latest endpoint is unavailable at [local-path] token=[redacted]",
    });
  ...
  it("records degraded analysis readiness when chatfix fallback succeeds after missing Gemma primary", async () => {
    await upsertSelfImprovementRecommendations({
      stateDir: tmpDir,
      recommendations: [recommendation()],
    });
