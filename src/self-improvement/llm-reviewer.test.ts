...
describe("self-improvement LLM reviewer", () => {
  ...
  it("selects Gemma Q8 as the default local-first review model", async () => {
    const calls: Array<{
      modelId?: string;
      maxTokens: number;
      temperature: number;
  ...
  it("keeps oversized reviewer input valid and whole-group bounded", () => {
    const groups = Array.from({ length: 8 }, (_, index) =>
      group({
        id: `sig_large_${index}`,
        topEvidence: Array.from({ length: 4 }, () => "evidence ".repeat(600)),
        analysis: {
          ...group().analysis,
          summary: "summary ".repeat(300),
        },
        recommendedAction: "action ".repeat(300),
      }),
    );

    const payload = buildBoundedReviewPayload(groups);
    const serialized = JSON.stringify(payload);

    expect(serialized.length).toBeLessThanOrEqual(12_100);
    expect(JSON.parse(serialized)).toEqual(payload);
    expect(payload.groups.every((entry) => entry.id.startsWith("sig_large_"))).toBe(true);
    expect(payload.omittedGroupCount).toBeGreaterThan(0);
  });
  ...
  it("labels responsive default Ollama reviewers when no explicit provider config exists", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        return new Response(
          JSON.stringify({
            models: [
              { name: "openclaw-control-gemma4-31b-q8:latest" },
    ...
    expect(result).toMatchObject({
      ...
      readyModelId: "ollama/openclaw-control-gemma4-31b-q8:latest",
    });
  ...
  it("preflights local-first review models without generation", async () => {
    ...
    expect(result).toMatchObject({
      ...
      preflightStatus: "missing_config",
      preflightMs: 4,
      blockedPrimaryReason: "ollama/openclaw-control-gemma4-31b-q8:latest is not configured",
    });
    expect(result.attempts).toMatchObject([
      { tier: "primaryReview", status: "blocked", preflightStatus: "missing_config" },
      { tier: "crossCheck", status: "blocked", preflightStatus: "missing_config" },
  ...
  it("marks model preflight degraded when Gemma primary is blocked but chatfix fallback is ready", async () => {
    const result = await preflightSelfImprovementReviewModels({
      cfg: {} as OpenClawConfig,
      localFirst: true,
      checkedAt: now,
      preflight: async (params) => {
        if (params.modelId === "ollama/openclaw-control-gemma4-31b-q8:latest") {
          return {
            ok: false,
            status: "missing_config",
            elapsedMs: 2,
            reason: "Local model preflight could not find openclaw-control-gemma4-31b-q8:latest.",
          };
        }
        return {
          ok: true,
  ...
  it("blocks local readiness when chatfix is reachable but invalid", async () => {
    const result = await reviewSelfImprovementGroupsWithLlm({
      groups: [group()],
      requested: false,
      localFirst: true,
  ...
  it("uses strategic review only after primary and fallback fail for major-change groups", async () => {
