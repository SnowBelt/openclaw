...
const LLM_PROMPT_VERSION = "self-improvement-governor-llm-review-v3";
...
export function buildBoundedReviewPayload(groups: readonly SelfImprovementRecommendationGroup[]): {
  groups: ReturnType<typeof buildReviewPayload>["groups"];
  omittedGroupCount: number;
} {
  const candidates = buildReviewPayload(groups).groups;
  const selected: typeof candidates = [];
  for (const candidate of candidates) {
    const next = { groups: [...selected, candidate] };
    if (JSON.stringify(next).length <= MAX_PROMPT_CHARS) {
      selected.push(candidate);
      continue;
    }
    if (selected.length === 0) {
      const compactCandidate = { ...candidate, evidence: [] };
      if (JSON.stringify({ groups: [compactCandidate] }).length <= MAX_PROMPT_CHARS) {
        selected.push(compactCandidate);
      }
    }
    break;
  }
  return {
    groups: selected,
    omittedGroupCount: Math.max(0, groups.length - selected.length),
  };
}
...
function buildUserPrompt(groups: readonly SelfImprovementRecommendationGroup[]): string {
  const boundedPayload = JSON.stringify(buildBoundedReviewPayload(groups));
  return [
    ...
    "Some lower-ranked groups may be omitted when the bounded input budget is reached.",
    "Input:",
    boundedPayload,
  ].join("\n");
}
