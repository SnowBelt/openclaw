...
function parseRecommendation(value: unknown): SelfImprovementRecommendation | null {
  ...
  const resolutionProof =
    typeof entry.resolutionProof === "string"
      ? sanitizeRecommendationText(entry.resolutionProof, 640)
      : undefined;
  const storedEvidenceKeys = normalizeStringArray(entry.observedEvidenceKeys);
  const observedEvidenceKeys =
    storedEvidenceKeys.length > 0
      ? storedEvidenceKeys
  ...
  const lastNovelEvidenceAt =
    typeof entry.lastNovelEvidenceAt === "number" && Number.isFinite(entry.lastNovelEvidenceAt)
      ? entry.lastNovelEvidenceAt
      : updatedAt;
