...
export function filterSelfImprovementSystemTasks(tasks: TaskRecord[]): TaskRecord[] {
  return tasks.filter(
    (task) =>
      task.taskKind !== "self-improvement" &&
      task.sourceId !== "self-improvement-governor" &&
      task.ownerKey !== "system:self-improvement" &&
      task.requesterSessionKey !== "system:self-improvement",
  );
}
