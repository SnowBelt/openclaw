...
describe("self-improvement recommendation store", () => {
  ...
  it("preserves acknowledged status and reopens resolved findings only for novel evidence", async () => {
    await upsertSelfImprovementRecommendations({
      stateDir: tmpDir,
      recommendations: [recommendation()],
    });
    ...
    const unchanged = await upsertSelfImprovementRecommendations({
      stateDir: tmpDir,
      recommendations: [recommendation({ title: "Recommendation recurred" })],
    });

    expect(unchanged.reopened).toBe(0);
    expect(unchanged.recommendations[0]?.status).toBe("resolved");
    expect(unchanged.recommendations[0]?.recurrenceCount).toBe(1);
  });

  it("reopens only when new evidence arrives and marks old proof stale", async () => {
    const first = recommendation({ observedEvidenceKeys: ["old-evidence"] });
    await upsertSelfImprovementRecommendations({ stateDir: tmpDir, recommendations: [first] });
    await updateSelfImprovementRecommendationStatus({
      stateDir: tmpDir,
      id: "sir_test",
      status: "resolved",
    ...
    const reopened = await upsertSelfImprovementRecommendations({
      ...
      recommendations: [
        recommendation({
          ...
          updatedAt: Date.parse("2026-05-07T12:30:00.000Z"),
          lastSeenAt: Date.parse("2026-05-07T12:30:00.000Z"),
        }),
      ],
    });

    expect(reopened.reopened).toBe(1);
    expect(reopened.recommendations[0]).toMatchObject({
      status: "reopened",
      recurrenceCount: 2,
      resolutionProof: "pnpm test passed",
  ...
  it("derives evidence metadata when loading an older store", async () => {
    const storePath = resolveSelfImprovementRecommendationStorePath(tmpDir);
    await fs.mkdir(path.dirname(storePath), { recursive: true });
    await fs.writeFile(
      storePath,
      JSON.stringify({ version: 2, recommendations: [recommendation()] }, null, 2),
    );

    await expect(
      getSelfImprovementRecommendation({ stateDir: tmpDir, id: "sir_test" }),
    ).resolves.toMatchObject({
      observedEvidenceKeys: expect.arrayContaining([expect.any(String)]),
      lastNovelEvidenceAt: expect.any(Number),
    });
  });
