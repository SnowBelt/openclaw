...
vi.mock("./overview.js", () => ({
  formatCrestodianOverview: () => "Default model: openai/gpt-5.6-luna",
...
describe("parseCrestodianOperation", () => {
  ...
  it("runs setup bootstrap only after approval and audits it", async () => {
    ...
    expect(lines.join("\n")).toContain("Model choice: openai/gpt-5.6-luna (OPENAI_API_KEY).");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Bootstrapped setup with openai/gpt-5.6-luna",
      },
      {
        rescue: true,
        workspace: "/tmp/work",
