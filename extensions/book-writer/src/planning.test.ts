...
describe("book-writer planning studio model", () => {
  ...
  it("keeps operator continuity controls in paragraph and chapter rewrite context", async () => {
    const outputDir = await tempOutputDir();
    const config = resolveBookWriterConfig({ outputDir });
    const plan = await createAndSaveBookPlan({
      config,
    ...
    const characterFact = "Mara Vale never lies to her sister Elian, even under pressure.";
    const timelineEvent = "The bridge inspection happens before the council vote, never after it.";
    const toneRule = "Keep suspense dry and precise; never turn the scene into slapstick.";
    const plotDirection = "The forged signature must pay off in the final public reveal.";
    const saved = await saveBookPlan({
      config,
      baseVersion: plan.version,
      action: "seed-continuity-controls",
      summary: "Seed operator continuity controls.",
      plan: {
        ...plan,
        continuityControl: {
    ...
    const paragraph = chapter.paragraphs[1];
    const paragraphPacket = buildParagraphContextPacket({
      plan: saved,
      chapter,
      paragraph,
      previous: chapter.paragraphs[0],
    ...
    const paragraphPrompt = buildParagraphRewritePrompt(paragraphPacket);
    const chapterPrompt = buildChapterRewritePrompt(chapterPacket);
    const memory = buildHierarchicalMemory(saved);

    expect(saved.continuityControl?.characterFacts).toContain(characterFact);
    expect(saved.bookSync?.state).toBe("needs-propagation");
    expect(saved.bookSync?.affectedChapterIds).toHaveLength(saved.chapters.length);
    expect(saved.storyImpactEvents?.at(-1)?.editSummary).toContain(
      "Operator continuity controls changed",
    );
    expect(memory.characterBible.map((character) => character.state).join(" ")).toContain(
      characterFact,
    );
    expect(memory.timeline.map((event) => event.event).join(" ")).toContain(timelineEvent);
    expect(paragraphPacket.relevantCharacters.join(" ")).toContain(characterFact);
    expect(paragraphPacket.relevantTimelineFacts.join(" ")).toContain(timelineEvent);
    expect(paragraphPacket.styleGuide.join(" ")).toContain(toneRule);
    expect(paragraphPacket.futureConsequences.join(" ")).toContain(plotDirection);
    expect(chapterPacket.characterArcs.join(" ")).toContain(characterFact);
    expect(chapterPacket.timeline.join(" ")).toContain(timelineEvent);
    expect(chapterPacket.styleGuide.join(" ")).toContain(toneRule);
    expect(chapterPacket.setupPayoffNeeds.join(" ")).toContain(plotDirection);
    expect(paragraphPrompt).toContain(characterFact);
    expect(paragraphPrompt).toContain(timelineEvent);
    expect(chapterPrompt).toContain(toneRule);
    expect(chapterPrompt).toContain(plotDirection);
  });
  ...
  it("uses full paragraph context packets when AI adapts propagated story changes", async () => {
    const outputDir = await tempOutputDir();
    const config = resolveBookWriterConfig({ outputDir });
    const plan = await createAndSaveBookPlan({
      config,
    ...
    const edited = await saveBookPlan({
      config,
      baseVersion: plan.version,
      action: "seed-operator-sentence-change",
      summary: "Seed a sentence that changes the whole story.",
      plan: twistPlan,
    });
    ...
    expect(prompts[0]).toContain("Expert role:");
    expect(prompts[0]).toContain("Selected paragraph:");
    expect(prompts.some((prompt) => prompt.includes("Previous paragraph:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Next paragraph:"))).toBe(true);
    expect(prompts[0]).toContain("Main storyline tracker:");
    expect(prompts[0]).toContain("Relevant timeline facts:");
    expect(prompts[0]).toContain("Future consequences:");
    expect(prompts[0]).toContain("Locked text rules:");
    expect(prompts[0]).toContain("Rewrite this paragraph for story-impact propagation");
    const rewritten = propagated.bookSync?.affectedParagraphIds ?? [];
    expect(
      propagated.chapters
        .flatMap((chapter) => chapter.paragraphs)
        .filter((paragraph) => rewritten.includes(paragraph.id))
        .some((paragraph) => paragraph.text.includes("rain-dark bridge")),
    ).toBe(true);
  });
  ...
  it("drafts and stitches a cohesive multi-chapter story while preserving locked text exactly", async () => {
    ...
    expect(
      prompts.some(
        (prompt) => prompt.includes("Previous chapter 1") && prompt.includes("Next chapter 3"),
      ),
    ).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Chapter beat map:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Chapter draft so far:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("not isolated fragments"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes(lockedText))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Locked constraints:"))).toBe(true);
