...
function continuityControl(plan: BookPlan): Required<NonNullable<BookPlan["continuityControl"]>> {
  return {
...
function operatorCharacterLines(plan: BookPlan): string[] {
  return continuityControl(plan).characterFacts.map((fact) => `Operator character fact: ${fact}`);
}

function operatorTimelineLines(plan: BookPlan): string[] {
  return continuityControl(plan).timelineEvents.map((event) => `Operator timeline event: ${event}`);
}

function operatorToneLines(plan: BookPlan): string[] {
  return continuityControl(plan).toneRules.map((rule) => `Operator tone rule: ${rule}`);
}

function operatorPlotLines(plan: BookPlan): string[] {
  return continuityControl(plan).plotDirections.map(
    (direction) => `Operator plot direction: ${direction}`,
  );
}
...
export function buildHierarchicalMemory(
  ...
  const manualTimeline = operatorTimelineLines(plan).map((event, index) => ({
    order: 50 + index,
    chapterId: "operator-story-bible",
    event,
    consequence: "All generated or edited text must preserve this operator-specified chronology.",
  }));
  ...
  return {
    ...
    characterBible: [
      ...inferCharacters(plan),
      ...operatorCharacterLines(plan).map((fact, index) => ({
        name: `Operator Story Bible ${index + 1}`,
        role: "unknown" as const,
        goals: ["Preserve this user-defined character continuity fact."],
        motivations: [fact],
        arc: fact,
        state: fact,
        evidence: [fact],
    ...
    timeline: [...manualTimeline, ...timeline],
    chapterMap,
    sceneMap,
    styleGuide: {
      tone: plan.styleGuide?.toneDescription ?? plan.brief.tone,
...
export function buildParagraphContextPacket(params: {
  ...
  return {
    ...
    relevantCharacters: uniqueValues(
      [
        ...relevantCharacterLines(
          memory,
          `${params.paragraph.text} ${params.paragraph.summary} ${params.chapter.description}`,
        ),
        ...operatorCharacterLines(params.plan),
      ],
      16,
    ),
    relevantTimelineFacts: uniqueValues(
      [
        ...relevantTimelineLines(memory, params.chapter.id, params.paragraph.id),
        ...operatorTimelineLines(params.plan),
      ],
      16,
    ),
    styleGuide: uniqueValues(
      [...memory.styleGuide.proseRules, ...operatorToneLines(params.plan)],
      20,
    ),
    futureConsequences: [
...
export function buildChapterContextPacket(params: {
  ...
  return {
    ...
    characterArcs: uniqueValues(
      [
        ...memory.characterBible.map(
    ...
    timeline: uniqueValues(
      [...relevantTimelineLines(memory, params.chapter.id), ...operatorTimelineLines(params.plan)],
      20,
    ),
    ...
    setupPayoffNeeds: uniqueValues(
      [
        ...(chapterMemory?.setupPayoffNeeds ?? [
