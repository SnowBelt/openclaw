...
const LOCAL_AI_HEALTH_TIMEOUT_MS = 1500;

async function fetchJsonWithTimeout(fetchImpl: typeof fetch, url: string): Promise<unknown> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), LOCAL_AI_HEALTH_TIMEOUT_MS);
  try {
...
async function readLocalCoverAiStatus(
  ...
  if (missingComfyConfig.length > 0) {
    return {
      status: "fallback",
      provider: "comfy",
      model: configuredRef,
  ...
  const configured = comfy?.isConfigured?.({ cfg: appConfig }) ?? Boolean(comfy);
  if (!configured) {
    return {
      status: "setup-needed",
...
async function generateAndSaveLocalAiCoverImage(params: {
  ...
  const localStatus = await readLocalCoverAiStatus(params.appConfig, params.fetchImpl);
