...
function normalizeContinuityList(values: unknown, limit = 8): string[] {
...
function buildContinuityControl(params: {
  topic: string;
  title: string;
  tone: string;
  existing?: Partial<BookPlanContinuityControl>;
}): BookPlanContinuityControl {
  const topic = params.topic.replace(/\s+/g, " ").trim();
  const title = params.title.replace(/\s+/g, " ").trim() || "this book";
  const characterFacts = normalizeContinuityList(params.existing?.characterFacts);
  const timelineEvents = normalizeContinuityList(params.existing?.timelineEvents);
  const toneRules = normalizeContinuityList(params.existing?.toneRules);
  const plotDirections = normalizeContinuityList(params.existing?.plotDirections);
  return {
    characterFacts: characterFacts.length
      ? characterFacts
      : [`Main cast must stay consistent with the book idea: ${topic || title}.`],
    timelineEvents: timelineEvents.length
      ? timelineEvents
      : ["Maintain clear cause and effect from each chapter into the next."],
    toneRules: toneRules.length ? toneRules : [`Voice must stay aligned with: ${params.tone}.`],
    plotDirections: plotDirections.length
      ? plotDirections
      : ["Every chapter must move toward the promised ending or reader payoff."],
  };
}
...
function hydrateBookPlan(plan: BookPlan): BookPlan {
  ...
  return {
    ...plan,
    styleGuide,
    continuityControl: buildContinuityControl({
      topic: plan.brief.topicParagraph || plan.topic,
      title: plan.title,
      tone: styleGuide.toneDescription,
      existing: plan.continuityControl,
    }),
    brief: {
      ...plan.brief,
      tone: styleGuide.toneDescription,
...
async function draftBookPlanWithModel(params: {
  ...
  for (const [chapterIndex, chapter] of params.plan.chapters.entries()) {
    ...
    const batchGenerated = await generateChapterWindowBookText({
      ...
      plan: {
        ...params.plan,
        chapters: params.plan.chapters.map((candidateChapter) =>
          candidateChapter.id === currentChapter.id ? currentChapter : candidateChapter,
        ),
      },
...
async function rewritePropagatedParagraphsWithModel(params: {
  ...
  // The deterministic propagation pass already updates every affected editable paragraph.
  // Live model rewriting is a quality overlay for the highest-priority changed beats, so keep it
  // bounded enough that a dashboard sentence edit does not feel hung.
  const targetIds = new Set(params.paragraphIds.slice(0, 2));
  ...
  for (const chapter of params.plan.chapters) {
    ...
    for (const [paragraphIndex, paragraph] of chapter.paragraphs.entries()) {
      ...
      const packet = buildParagraphContextPacket({
        ...
        next: chapter.paragraphs[paragraphIndex + 1],
        userInstruction: [
          "Rewrite this paragraph for story-impact propagation after the operator changed the story.",
          "Make the inserted change affect only this editable paragraph's setup, bridge, payoff, or consequence duties.",
          "Use the previous paragraph, next paragraph, chapter purpose, whole-book storyline, timeline, future consequences, and locked-text rules.",
          "Preserve useful existing prose when it still fits, but remove contradictions created by the story change.",
          `Continuity obligations: ${(paragraph.continuityObligations ?? []).join(" ") || "Keep the propagated story change coherent."}`,
      ...
      const cohesion = text
        ...
        : undefined;
      nextParagraphs.push(
        text && !looksLikeInstructionalBookText(text) && (cohesion?.minimumScore ?? 0) >= 6
          ? {
              ...paragraph,
              text,
              status: cohesion?.status === "flag" ? "needs-revision" : "drafted",
              revisionStatus: cohesion?.status === "pass" ? "clean" : "needs-context-repair",
            }
          : paragraph,
      );
    }
...
export async function readManuscriptPreview(
  _config: ResolvedBookWriterConfig,
  plan: BookPlan | null,
): Promise<string> {
  if (!plan) {
    return "";
