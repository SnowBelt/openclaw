...
function continuityValues(plan: BookPlan): Record<string, string[]> {
  return {
    characterFacts: plan.continuityControl?.characterFacts ?? [],
    timelineEvents: plan.continuityControl?.timelineEvents ?? [],
    toneRules: plan.continuityControl?.toneRules ?? [],
    plotDirections: plan.continuityControl?.plotDirections ?? [],
  };
}

function changedContinuityControls(previous: BookPlan | null, next: BookPlan): string[] {
  if (!previous) {
    return [];
  }
  const before = continuityValues(previous);
  const after = continuityValues(next);
...
export function buildStoryImpactState(params: {
  ...
  const continuityChanges = params.suppressDetection
    ? []
    : changedContinuityControls(params.previous, params.plan);
  ...
  const continuityEvents =
    continuityChanges.length && params.plan.chapters[0]
      ? [
          {
            id: `impact-${shortHash(`${params.plan.runId}:${params.version}:continuity:${continuityChanges.join(",")}`)}`,
            createdAt: params.now.toISOString(),
            sourceVersion: params.version,
            sourceChapterId: params.plan.chapters[0].id,
            editSummary: `Operator continuity controls changed: ${continuityChanges.join(", ")}.`,
            impactClass: "book" as const,
            impactLevel: "whole-book" as const,
            twistTypes: continuityChanges.includes("plotDirections")
              ? ["changed_ending", "chapter_logic"]
              : ["chapter_logic"],
            affectedSystems: continuityChanges,
            affectedChapterIds: params.plan.chapters.map((chapter) => chapter.id),
            status: "detected" as const,
          },
        ]
      : [];
  const storyImpactEvents = [...priorEvents, ...newEvents, ...continuityEvents].slice(-50);
