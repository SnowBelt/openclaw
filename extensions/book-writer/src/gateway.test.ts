...
describe("book-writer gateway planning methods", () => {
  ...
  it("uses live gateway fetch when propagating a saved sentence-level story change", async () => {
    const outputDir = await tempOutputDir();
    const { api, registerGatewayMethod } = createApi();
    const prompts: string[] = [];
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = input instanceof Request ? input.url : input.toString();
      if (init?.method === "GET" && (url.endsWith("/models") || url.endsWith("/api/tags"))) {
        return new Response(JSON.stringify({ data: [{ id: "qwen2.5:32b" }], models: [] }), {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      }
          ...
          choices: [
            {
              message: {
                content:
                  "Mara crossed the rain-dark bridge with the invoice under her coat, aware that every bolt, ledger line, and quiet footstep now had to connect to the clerk's hidden motive. She carried the prior pressure forward, protected the next council-room clue, and made the changed relationship feel like consequence instead of a fragment.",
              },
            },
          ],
        }),
    ...
    const editedPlan = structuredClone(create.plan!);
    editedPlan.chapters[2].paragraphs[1].text =
      "Mara realized the trusted council clerk was secretly her sister and the real mastermind behind the forged bridge invoice.";
    editedPlan.chapters[2].paragraphs[1].status = "drafted";
    const saved = expectSnapshot(
      await invoke(findHandler(registerGatewayMethod, "bookWriter.plan.save"), {
        plan: editedPlan,
        baseVersion: create.plan!.version,
      }),
    );

    const propagated = expectSnapshot(
      await invoke(findHandler(registerGatewayMethod, "bookWriter.plan.propagateStoryChange"), {
        runId: saved.plan!.runId,
        baseVersion: saved.plan!.version,
      }),
    );

    expect(propagated.plan?.bookSync?.state).toBe("fully-updated");
    expect(prompts.some((prompt) => prompt.includes("Expert role:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Previous paragraph:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Future consequences:"))).toBe(true);
    expect(prompts.some((prompt) => prompt.includes("Locked text rules:"))).toBe(true);
  });
  ...
  it("fails local cover AI health checks fast enough to keep cover controls recoverable", async () => {
    const outputDir = await tempOutputDir();
    const { api, registerGatewayMethod } = createApi();
    api.config = {
      agents: {
    ...
    expect(localStatus).toMatchObject({
      status: "setup-needed",
      message: expect.stringContaining("ComfyUI is configured but not reachable"),
    });
    expect(fetchImpl).toHaveBeenCalledWith(
